...
func TestV2MessageTypeIDsAreStable(t *testing.T) {
	require.Equal(t, uint16(101), V2TypeLogin)
	require.Equal(t, uint16(102), V2TypeLoginResp)
	require.Equal(t, uint16(103), V2TypeNewProxy)
	require.Equal(t, uint16(104), V2TypeNewProxyResp)
	require.Equal(t, uint16(105), V2TypeCloseProxy)
	require.Equal(t, uint16(106), V2TypeNewWorkConn)
	require.Equal(t, uint16(107), V2TypeReqWorkConn)
	require.Equal(t, uint16(108), V2TypeStartWorkConn)
	require.Equal(t, uint16(109), V2TypeNewVisitorConn)
	require.Equal(t, uint16(110), V2TypeNewVisitorConnResp)
	require.Equal(t, uint16(111), V2TypePing)
	require.Equal(t, uint16(112), V2TypePong)
	require.Equal(t, uint16(113), V2TypeUDPPacket)
	require.Equal(t, uint16(114), V2TypeNatHoleVisitor)
	require.Equal(t, uint16(115), V2TypeNatHoleClient)
	require.Equal(t, uint16(116), V2TypeNatHoleResp)
	require.Equal(t, uint16(117), V2TypeNatHoleSid)
	require.Equal(t, uint16(118), V2TypeNatHoleReport)
}
